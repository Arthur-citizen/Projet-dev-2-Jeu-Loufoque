...
wav = ["assets/musiques/emotional.wav",
...
class Plateau:
    def __init__(self, nbCases):
        ...
        self.defi = ["Fais 5 pompes, histoire de te décoller le derrière de ta chaise. :-)",
                     ...
                     "Allez, bouge un peu et tiens en équilibre sur une jambe pendant dix secondes. Je suis sympa, je te laisse choisir laquelle. ';-)"]
    ...
    def create_cases(self):
        ...
        probabilites = [1 - miniProb - eventProb, miniProb,
                        eventProb]  # probabilité associé au différente possibilié, il faudrait en faire un dictionnaire

        for i in range(self.nbCases + 2):
            type = random.choices(possiblilite, weights=probabilites, k=1)[
                0]  # choisis un type de cases alléatoirement en fonction des probabilité
            self.cases.append(Case(type))
            if type == "JEU":
                probabilites[0], probabilites[1], probabilites[2] = 1 - miniProb - (
                # Ca caluc les prob, si un type est choisis ça met ses prob
                        probabilites[2] + eventProb), miniProb, probabilites[
                                                                        2] + eventProb  # par default sinon àa rajoute ses prob par defaut a sa prob actuel
            elif type == "EVENT":
            ...
            while tentative < 100:
                try:
                    ...
                    for cas in self.cases:
                        ...
                        nextPos = [(pos[0] + 1, pos[1]), (pos[0] - 1, pos[1]), (pos[0], pos[1] + 1),
                                   (pos[0], pos[1] - 1)]  # postions des cases dans les 4 directions
    ...
    def afficherPlateau(self):
        ...
        self.canvas.create_oval(joueurX - self.tailleCase // 4, joueurY - self.tailleCase // 4,
                                joueurX + self.tailleCase // 4, joueurY + self.tailleCase // 4,
    ...
    def lancerDe(self):
        ...
        """
        num = random.randint(1, 6)
        self.joueur.position += (num if (self.joueur.position + num) < len(self.cases) else len(self.cases) - 1 - self.joueur.position)
        self.afficherPlateau()
    ...
    def afficherPlateau(self):
        ...
        joueurY = self.cases[self.joueur.position].position[0] * self.tailleCase + self.tailleCase // 2
        joueurX = self.cases[self.joueur.position].position[1] * self.tailleCase + self.tailleCase // 2
        self.canvas.create_oval(joueurX - self.tailleCase // 4, joueurY - self.tailleCase // 4,
                                joueurX + self.tailleCase // 4, joueurY + self.tailleCase // 4,
                                fill=self.__couleurs["JOUEUR"])
